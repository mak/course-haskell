...
import System.Posix (setFdOption,FdOption(..),closeFd)
...
fs2 :: Term =
       Folder $ Map.fromList [("d1",fs2), ("fl1", File "File1")]
...
osloop world = 
  ...
  try'to'run qe@(JQBlockedOnRead ctx@ProcessCTX{psocket=s} k) world =
      do
...
interpret'req :: World r -> ProcessCTX -> OSReq r IO -> CC r IO (World r)
...
run'process :: (forall m. Monad m =>
		(Prompt r (OSReq r m)) -> CC r m (OSReq r m))
	    -> Prompt r (OSReq r IO) -> CC r IO (OSReq r IO)
...
fsProcess :: Monad m => 
	     CC r m (FSZipper r m) -> Prompt r (OSReq r m) 
	  -> CC r m (OSReq r m)
...
fsCommands = 
    [
     ("quit", \z svcp _ _ _ -> svc svcp $ const OSRDone),
     ("cd", fsWrapper
     ...
     ("commit",  fcmd'commit),
     ("refresh", \z svcp _ _ rest -> svc svcp OSRefresh >>=
...
-- A delimited continuation suffice!
...
cmd'help z shp _ _ = return $ FSCS $ "Commands: " ++ 
...
cd'zipper DZipper{dz_term = File _} shp _ =
    abortP shp (return $ FSCS "cannot descend down the file")
cd'zipper DZipper{dz_term = Folder fld, dz_k = k} shp path 
    = let (pc,prest) = breakspan (== '/') path
	 ...
	 else abortP shp (return $ FSCS $ "No such dir component " ++ pc)
...
cmd'mknode _ _ shp _ dirn | '/' `elem` dirn =
    abortP shp (return $ FSCS "the name of the new node can't contain slash")
cmd'mknode _ _ shp _ "" =
    abortP shp (return $ FSCS "the name of the new node is empty")
cmd'mknode newnode DZipper{dz_term = File _} shp _ _ =
    abortP shp (return $ FSCS "cannot create anything in a file")
cmd'mknode newnode DZipper{dz_term = Folder fld} shp _ dirn
    | Map.member dirn fld =
	abortP shp (return $ FSCS $ "node " ++ dirn ++ " already exists")
cmd'mknode newnode DZipper{dz_term = Folder fld, dz_k = k, dz_dir = cn} 
   shp _ dirn =
    let fld' = Folder $ Map.insert dirn newnode fld
    in k (return (Just fld',Up)) >>= adjust cn >>= return . FSCZ 
  where
  -- go back to the current directory
  adjust _ (DZipDone term) = dzip'term term
  adjust cn z = dz_k z (return (Nothing,cn))
...
del'zipper z shp path = cd'zipper z shp path >>= 
  ...
  where
  aux _ (DZipDone _) = 
      abortP shp (return $ FSCS $ "cannot remove the root folder")
...
cmd'mv z shp _ args = aux $ breakspan is'whitespace args
  where
  aux ("",_) = abortP shp (return $ FSCS $ "mv: from-path is empty")
  aux (_,"") = abortP shp (return $ FSCS $ "mv: to-path is empty")
...
cmd'cp z0 shp _ args = aux $ breakspan is'whitespace args
  where
  aux ("",_) = abortP shp (return $ FSCS $ "cp: from-path is empty")
  aux (_,"") = abortP shp (return $ FSCS $ "cp: to-path is empty")
